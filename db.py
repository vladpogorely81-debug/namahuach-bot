...
class Database:
    ...
    def register_user(self, user_id, name, username):
        with self.conn() as c:
            c.execute("INSERT OR IGNORE INTO users (user_id, name, username) VALUES (?,?,?)", (user_id, name, username))
            c.execute("UPDATE users SET name=?, username=? WHERE user_id=?", (name, username, user_id))
    ...
    def get_all_teams(self):
        with self.conn() as c:
            rows = c.execute("SELECT DISTINCT team1 FROM matches UNION SELECT DISTINCT team2 FROM matches ORDER BY 1").fetchall()
    ...
    def get_all_matches_with_bets(self, user_id):
        with self.conn() as c:
            rows = c.execute("""
            ...
            for r in rows:
                d = dict(r)
                d['user_bet'] = f"{d['bet1']}:{d['bet2']}" if d.get('bet1') is not None else None
                result.append(d)
            return result
    ...
    def get_open_matches_for_betting(self, user_id):
        with self.conn() as c:
            return c.execute("""
                ...
                WHERE m.is_open = 1 AND m.score1 IS NULL
    ...
    def get_user_bet(self, user_id, match_id):
        with self.conn() as c:
            return c.execute("SELECT * FROM bets WHERE user_id=? AND match_id=?", (user_id, match_id)).fetchone()

    def place_bet(self, user_id, match_id, bet1, bet2):
        with self.conn() as c:
            c.execute("INSERT INTO bets (user_id, match_id, bet1, bet2) VALUES (?,?,?,?)", (user_id, match_id, bet1, bet2))

    def update_bet(self, user_id, match_id, bet1, bet2):
        with self.conn() as c:
            c.execute("UPDATE bets SET bet1=?, bet2=?, result_delta=NULL WHERE user_id=? AND match_id=?", (bet1, bet2, user_id, match_id))

    def get_match_bet_count(self, match_id):
        with self.conn() as c:
    ...
    def get_matches_pending_result(self):
        with self.conn() as c:
            return c.execute("SELECT * FROM matches WHERE score1 IS NULL ORDER BY id").fetchall()

    def get_match_bets_preview(self, match_id):
        with self.conn() as c:
            return c.execute("""
                SELECT b.bet1, b.bet2, u.name FROM bets b
                JOIN users u ON u.user_id = b.user_id
                WHERE b.match_id = ? ORDER BY u.name
            """, (match_id,)).fetchall()
    ...
    def set_match_result(self, match_id, s1, s2):
        with self.conn() as c:
            c.execute("UPDATE matches SET score1=?, score2=?, is_open=0 WHERE id=?", (s1, s2, match_id))
    ...
    def calculate_and_save_results(self, match_id, s1, s2):
        with self.conn() as c:
            ...
            for b in bets:
                ...
                c.execute("UPDATE bets SET result_delta=?, result_label=? WHERE id=?", (res['delta'], res['label'], b['id']))
                results.append({'user_id': b['user_id'], 'name': b['name'], 'bet1': b['bet1'], 'bet2': b['bet2'], 'delta': res['delta'], 'label': res['label']})
            return results

    # ── CHAMPION BETS ──────────────────────────────────────────────────────────

    def place_champion_bet(self, user_id, team, amount):
        with self.conn() as c:
            c.execute("INSERT OR REPLACE INTO champion_bets (user_id, team, amount) VALUES (?,?,?)", (user_id, team, amount))

    def get_champion_bet(self, user_id):
        with self.conn() as c:
            return c.execute("SELECT * FROM champion_bets WHERE user_id=?", (user_id,)).fetchone()

    def get_all_champion_bets(self):
        with self.conn() as c:
            return c.execute("""
                SELECT cb.*, u.name FROM champion_bets cb
                JOIN users u ON u.user_id = cb.user_id
                ORDER BY cb.amount DESC
            """).fetchall()

    def calculate_champion_results(self, winner_team):
        with self.conn() as c:
            bets = c.execute("""
                SELECT cb.*, u.name FROM champion_bets cb
                JOIN users u ON u.user_id = cb.user_id
            ...
            for b in bets:
                if b['team'] == winner_team:
                    delta = b['amount']  # виграш х2 (повернення + виграш)
                else:
                    delta = -b['amount']
                c.execute("UPDATE champion_bets SET result_delta=? WHERE user_id=?", (delta, b['user_id']))
                results.append({'user_id': b['user_id'], 'name': b['name'], 'team': b['team'], 'amount': b['amount'], 'delta': delta})
            return results
    ...
    def get_standings(self):
        with self.conn() as c:
            return c.execute("""
                SELECT u.name,
                    COALESCE(SUM(b.result_delta), 0) + COALESCE(cb.result_delta, 0) as balance,
                    COUNT(b.id) as total_bets,
                    cb.team as champ_team
                FROM users u
                LEFT JOIN bets b ON b.user_id = u.user_id
                LEFT JOIN champion_bets cb ON cb.user_id = u.user_id
                GROUP BY u.user_id
    ...
    def get_bank_total(self):
        with self.conn() as c:
            row = c.execute("SELECT COALESCE(-SUM(result_delta), 0) as bank FROM bets WHERE result_delta IS NOT NULL").fetchone()
            champ_row = c.execute("SELECT COALESCE(-SUM(result_delta), 0) as bank FROM champion_bets WHERE result_delta IS NOT NULL").fetchone()
            return max(0, (row['bank'] if row else 0) + (champ_row['bank'] if champ_row else 0))
